...
import { getToken, setToken, removeToken, getTokenExprieTime, setTokenExprieTime } from '@/utils/auth'
  ...
  response => {
    ...
    if (res.code !== 200) {
      ...
      if (res.code === 50083) {
        // 请自行在引入 MessageBox
        // import { Message, MessageBox } from 'element-ui'
        MessageBox.confirm('您的账号已从别处登录，可以取消继续留在该页面，或者重新登录', '确定登出', {
          confirmButtonText: '重新登录',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
      ...
      if (res.code === 50081 || res.code === 50014 || res.code === 50015) {
        // 请自行在引入 MessageBox
        // import { Message, MessageBox } from 'element-ui'
        MessageBox.confirm('安全认证过期，可以取消继续留在该页面，或者重新登录', '安全认证过期', {
          confirmButtonText: '重新登录',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
